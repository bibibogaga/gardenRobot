...
void MainWindow::updateMap()
{
    ...
    {
      if(qnode.cloudFused_xyz.at(i).z < 4)
      {
        x[i] = 0;
        y[i] = 0;
      }else{
...
{
  Quaterniond quaternion(qnode.carTF.pose.orientation.w,
                         qnode.carTF.pose.orientation.x,
                         qnode.carTF.pose.orientation.y,
                         qnode.carTF.pose.orientation.z);
  ...
  for (int i=0; i<qnode.cloud_xyz.width; i++)
  {
    ...
    x[i] = position[0] + qnode.carTF.pose.position.x;
    y[i] = position[1] + qnode.carTF.pose.position.y;
  }
